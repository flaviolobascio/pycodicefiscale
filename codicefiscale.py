...
# PATTERN to validate fiscal codes generated by Agenzia delle Entrate to avoid omocodie (http://www.agenziaentrate.gov.it/wps/content/Nsilib/Nsi/Schede/Istanze/Richiesta+TS_CF/Informazioni+codificazione+pf/?page=istanzecomunicazionicitt)
# checked at https://telematici.agenziaentrate.gov.it/VerificaCF/Scegli.do?parameter=verificaCf submitting this kind of fiscal codes
PATTERN = "^[A-Z]{6}[0-9LMNPQRSTUV]{2}([A-E]|[HLMPRST])[0-7LMNPQRST]{1}[0-9LMNPQRSTUV]{1}[A-Z][0-9LMNPQRSTUV][0-9LMNPQRSTUV][0-9LMNPQRSTUV][A-Z]$"
...
def isvalid(code):
    ...
    eg: isvalid('RCCMNL83S18D969H') -> True
        isvalid('RCCMNL83S18D969N') -> False
        isvalid('RCCMNL83S18D969') -> False
    """
    return isinstance(code, basestring) and re.match(PATTERN, code) is not None and control_code(code[0:15]) == code[15]
